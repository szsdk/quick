...
@click.option(
    "--tuple", type=(int, str), default=(23, "quick"), help="input (int, str)", nargs=2
)
...
@click.option(
    "--file_path", type=click.Path(file_okay=True, dir_okay=False), help="select a file"
)
@click.option(
    "--no_exist_path",
...
@cli.command(
    cls=quick.GCommand,
    new_thread=True,
    help="test for new_thread which creates a worker threads for called command",
)
@click.option(
    "--sleep_time",
